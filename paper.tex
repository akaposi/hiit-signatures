...
metatheoretical counterpart: types to types, universe to universe,
functions to functions, and so on.
...
Unfortunately, set-truncation makes it impossible to interpret syntactic types
as elements of a UIP-free metatheoretic $\mathsf{Type}_i$ universe. This is
because we must provide interpretations for all set-truncation constructors,
which amounts to shoving that the interpretations of $\Ty$, $\Con$, $\Sub$ are
all sets. However, we cannot show $\mathsf{Type}_i$ to be a set without UIP.
...
propositions, and metatheoretic equality types are not propositions in general.
...
\item Define a \emph{source} and a \emph{target} syntax in any suitable metatheory,
      ...
      do no necessarily need to differ.
...
\begin{alignat*}{5}
  ...
  & \equiv \,\, && \hat{\top}\timesh\hat{(}\hat{S^{1D}}\in \hat{S^1}\rah\Type_i\hat{)}\timesh\hat{(}\hat{b^D}\in \hat{S^{1D}\,b}\hat{)}\timesh\hat{(}\hat{loop^D\in \TR_{\,S^{1D}}\,loop\,b^D = b^D}\hat{)}
\end{alignat*}
...
TODO: REWRITE to remove redundacy from homomorphism section.
...
$\blank^\D$ translation.
\[
\infer{\hat{\Gamma}\semicol\Delta^\S \in \hat{(}\hat{\gamma}\in\Delta^\A\hat{)}\rah \Delta^\D\,\hat{\gamma}\rah \Type_{i}}{\hat{\Gamma}\vdash\Delta}
\]
...
context.
\[
\infer{\hat{\Gamma}\vdashh A^\S \in \hat{(}\hat{\gamma}\in\Delta^\A\hat{)}\hat{(}\hat{\gamma^D}\in\Delta^\D\,\hat{\gamma}\hat{)}\hat{(}\hat{\gamma^S}\in\Delta^\S\,\hat{\gamma}\,\hat{\gamma^D}\hat{)}\hat{(}\hat{x}\in A^\A\,\hat{\gamma}\hat{)}\rah  A^\D\,\hat{\gamma}\,\hat{\gamma^D}\,\hat{x}\rah \Type_{i}}{\hat{\Gamma}\semicol\Delta\vdash A}
\]
...
\begin{alignat*}{5}
  ...
  & (\Delta,\,x:A)^\S\,(\hat{\gamma},\hat{t})\,(\hat{\gamma^D},\hat{t^D}) && :\equiv \hat{(}\hat{\gamma^S}\in\Delta^\S\hat{\gamma^2}\hat{)}\timesh A^\S\,\hat{\gamma^3}\,\hat{t}\,\hat{t^D} \\
  & x^\S\,\hat{\gamma^3} && :\equiv x^{\text{th}}\text{ component in } \hat{\gamma^S} \\ % TODO: De Bruijn
  & \U^\S\,\hat{\gamma^3}\,\hat{A}\,\hat{A^D} && :\equiv \hat{(}\hat{x}\in \hat{A}\hat{)}\rah  \hat{A^D}\,\hat{x} \\
  & (\underline{a})^\S\,\hat{\gamma^3}\,\hat{t}\,\hat{t^D} && :\equiv a^\S\,\hat{\gamma^3}\,\hat{t} \eqh \hat{t^D} \\
  & ((x:a)\ra B)^\S\,\hat{\gamma^3}\,\hat{f}\,\hat{f^D} && :\equiv \hat{(}\hat{x}\in a^\A\,\hat{\gamma}\hat{)}\rah  B^\S\,\hat{(}\hat{\gamma,x}\hat{)}\,\hat{(}\hat{\gamma^D,}\,a^\S\,\hat{\gamma^3}\,\hat{x}\hat{)}\,\hat{(}\hat{\gamma^S,}\,\hat{\refl}\hat{)} \\
  & && \hspace{8.4em} \hat{(}\hat{f}\,\hat{x}\hat{)}\,\hat{\big(}\hat{f^D}\,\hat{x}\,\hat{(}a^\S\,\hat{\gamma^3}\,\hat{x}\hat{)}\hat{\big)} \\
  & (t\,u)^\S\,\hat{\gamma^3} && :\equiv \hat{\J}\,\hat{(}t^\S\,\hat{\gamma^3}\,\hat{(} u^\A\,\hat{\gamma}\hat{)}\hat{)}\,\hat{(}u^\S\,\hat{\gamma^3}\hat{)} \\
  & ((\hat{x}\in \hat{A})\ra B)^\S\,\hat{\gamma^3}\,\hat{f}\,\hat{f^D} && :\equiv \hat{(}\hat{x}\in \hat{A}\hat{)}\rah B^\S\,\hat{\gamma^3}\,\hat{(}\hat{f}\,\hat{x}\hat{)}\,\hat{(}\hat{f^D}\,\hat{x}\hat{)} \\
  & (t\,\hat{u})^\S\,\hat{\gamma^3} && :\equiv t^\S\,\hat{\gamma^3}\,\hat{u} \\
  & (t=_a u)^\S\,\hat{\gamma^3}\,\hat{e} && :\equiv \TR\,\hat{(}t^\S\,\hat{\gamma^3}\hat{)}\hat{\big(}\TR\,\hat{(}u^\S\,\hat{\gamma^3}\hat{)}\,\hat{(}\apd\,\hat{(}a^\S\,\hat{\gamma^3}\hat{)}\,\hat{e}\hat{)}\hat{\big)} \\
  & (\refl_t)^\S\,\hat{\gamma^3} && :\equiv \hat{\J}\,\hat{\refl}\,\hat{(}t^\S\,\hat{\gamma^3}\hat{)} \\
  & (\J_{a\,t\,(x.z.p)}\,pr\,_u\,eq)^\S\,\hat{\gamma^3} && :\equiv \\
  & && \hspace{-10em}\hat{\J}\,\hat{\bigg(}\hat{\J}\,\hat{\Big(}\hat{\J}\,\hat{\big(}\hat{\J}\,\hat{(}\hat{\lambda}\,\hat{p^D}\,\hat{p^S}\,\hat{pr^D}\,\hat{pr^S}.\,\hat{pr^S}\hat{)}\,\hat{(}t^\S\,\hat{\gamma^3}\hat{)}\,\\
  & && \hspace{-9em}\hat{(}\hat{\mathsf{uncurry}}\,p^\D\,\hat{\gamma^2}\hat{)}\,\hat{(}\hat{\mathsf{uncurry}}\,p^\S\,\hat{\gamma^3}\hat{)}\,\hat{(}pr^\D\,\hat{\gamma^2}\hat{)}\,\hat{(}pr^\S\,\hat{\gamma^3}\hat{)}\hat{\big)}\,eq^\A\,\hat{\gamma}\hat{\Big)}\,u^\S\,\hat{\gamma^3}\hat{\bigg)}\,\hat{(}eq^\S\,\hat{\gamma^3}\hat{)} \\
  & (\J\beta_{a\,t\,(x.z.p)}\,pr)^\S\,\hat{\gamma^3} && :\equiv \\
  & && \hspace{-10em}\hat{\J}\,\hat{\big(}\hat{\J}\,\hat{(}\hat{\lambda}\,\hat{p^D}\,\hat{p^S}\hat{.}\,\hat{\refl}\hat{)}\,\hat{(}t^\S\,\hat{\gamma^3}\hat{)}\,\hat{(}\hat{\mathsf{uncurry}}\,p^\D\,\hat{\gamma^2}\hat{)}\,\hat{(}\hat{\mathsf{uncurry}}\,p^\S\,\hat{\gamma^3}\hat{)}\hat{\big)}\,\hat{(}pr^\S\,\hat{\gamma^3}\hat{)}\\
  & ((\hat{x}\in \hat{A})\ra b)^\S\,\hat{\gamma^3\,f\,t} && :\equiv b^\S\,\hat{\gamma^3}\,\hat{(}\hat{f}\,\hat{t}\hat{)} \\
  & (t\,\hat{u})^\S\,\hat{\gamma^3} && :\equiv \ap\,\hat{(}\hat{\lambda} \hat{f.}\hat{f}\,\hat{u}\hat{)}\,\hat{(}t^\S\,\hat{\gamma^3}\hat{)}
\end{alignat*}
%% \endgroup
