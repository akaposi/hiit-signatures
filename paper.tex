...
indcution.} We can support iterated equalities of point and path constructors
by closing the universe under equality type formation of point and path
constructors, together with a standard (although size-restricted) definition
of path induction. We can also introduce a different type former for
equalities between type constructors.
...
\]
Here, $leaf$ has a function type (6) and $node$ has a function type (3) with a
function type (7) in the domain. More generally, we can define $W$-types
...
The interpretation here is inspired by the univalence axiom, although the
translation does not require it. If we assume univalence in the external theory,
then $\hat{a =_{\Type_i} b}$ equalities in are equivalent to equivalences, which
contain $\hat{f : a \ra b}$ functions together with $\hat{\mathsf{isEqv}\,f}$
proofs. Hence, we can view the $\hat{e : \blc{a^\A}\,\gamma =
  \blc{a^\A}\,\gamma}$ and $\hat{e^D : \TR_{(\lambda\,A.\,A \ra
    \mathsf{Type_0})}\,e\,(\blc{a^\D}\,\gamma^D) = \blc{b^\D}\,\gamma^D}$ proofs
as function bundled together with witnesses of equivalence. Since
$\hat{\mathsf{isEqv}}$ is proof irrelevant, we only need to relate the
underlying functions. For $\hat{e}$, the underlying function is given just by
$\hat{\coe\,e}$. For $\hat{e^D}$, the underlying function is a bit more
complicated, since $\hat{e^D}$ is a path over a path. We can observe this in the
example for higher inductive integers:
\begin{alignat*}{5}
  ...
  & && \hat{\hspace{1em}\times\,
       (p^S : (\lambda\,x.\,p^S\,(\coe\,p\,x)) = (\lambda\,x.\,\TR\,p^D\,(\J\,(p^S\,x)\,p)))}
\end{alignat*}
