...
\emph{category} of algebras, with homomorphisms as morphisms, identity
morphisms, composition together with category laws. This is not without
difficulties.

We are working in an UIP-free setting. Here, standard definitions of categories
feature set-truncated morphisms \cite{HoTTbook}. But we have non-truncated
notions of algebras and morphisms, so we cannot use the standard
definitions. One solution is to simply use non-truncated categories; these have
been previously called ``precategories'' or ``wild categories''
...
about higher categories. We explain in the following. The logical next step
...
equal to $\hat{\lambda\,x.\, x}$. However, we need to preserve $\U = \U[\sigma]$
