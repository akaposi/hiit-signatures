...
For $a =_\U b$ equalities, let us consider here only the $(a =_\U b)^\S$ case
for the type former. The interpretation here is inspried by the univalence
axiom, although the translation does not require it. If we assume univalence,
then $\hat{a =_{\Type_i} b}$ equalities in the external theory are equivalent to
equivalences, which contain a function $\hat{f : a \ra b}$ and a proof
$\hat{\mathsf{isEqv}\,f}$. Since $\hat{\mathsf{isEqv}}$ is propositional (proof
irrelevant), we only need to say something about the underlying $\hat{f}$
function in the translation.

Let us consider $\blank^\S$ for higher inductive integers with the type
equality:
\begin{alignat*}{5}
